...
const Header = () => {
  ...
  return (
    <header className="header">
      <a href="/">
        {ui.theme === "dark" ? <LogoDesktop /> : <LogoDesktopDark />}
      </a>
      <div className="header_right">
        <h1 className="board_title">{board.data[board.selected].title}</h1>
        <button className="mobile_board_selector">
          Roadmap
          <Down />
        </button>
        <div className="header_right_right">
          ...
          >
            <Add style={{ width: "1.1rem", height: "1.1rem" }} />
            <span className="add_new_task_text">Add New Task</span>
          </button>
          <button className="more">
            <More
              style={{ transform: "scale(1.4)", fill: "var(--color-gray)" }}
            />
          </button>
        </div>
      </div>
